...
def worker(downloader, lat, long, targetFolder, startedTiles, totTiles, version):
    ...
    if not os.path.exists(datafile(lat, long, targetFolder)):
        if not create_degree(downloader, lat, long, targetFolder, 100, version):
...
if __name__ == '__main__':
    ...
    with ThreadPool(args.processes-1) as p:
        reslist = [p.apply_async(worker, args=(downloader, td[0], td[1], targetFolder, td[2], len(tileID), args.version)) for td in tileID]
